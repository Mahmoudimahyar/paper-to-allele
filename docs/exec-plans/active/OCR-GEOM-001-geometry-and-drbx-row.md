...
- [x] M5b (2026-09-05, evening) `ocr/lattice.py`: the stored rulings as a grid; template-placed virtual anchors and ruled-row binding in `extract_facts.bind_in_lattice` (+2,264 cells, 0 swapped); the grid places the DRB3/4/5 slot without a DRB1 pair (`drbx_ink_pass.py --geometry`) and the empty labelled cells (`cell_ink_pass.py`: measurement only, no fact — HA-012 asks the human whether a per-page paper measurement may be NOT_TESTED). Ink counts only glyph-shaped components, and a page whose own print the measure cannot see certifies nothing (the control). Adversarial review (5 lenses, 122 agents, 35 confirmed findings, 16 high) then closed the ways a wrong absence could be certified: the ruled-row retry is gated to forms that print the locus on their values, a vertical divides a row only if it crosses 60% of it, a band containing another ruling is refused, tilted rulings are read at the x asked about rather than collapsed to a mean, the DRB3/4/5 slot is the cell the token is NOT in (read from the grid, never assumed), a row whose token rests on the S-for-5 repair certifies nothing (KI-026), a virtual anchor's provenance is mapped back to the stored frame, and a measurement a later run cannot make is withdrawn. 41 tests. Feasibility measured first from stored data: 3,223 / 4,541 unanchored cells sit in a ruled row at the template's label position, 20,517 / 24,187 empty labelled cells hold no box.
...
- The form's template may place a label the recognizer could not read, but only where a ruled row exists at that position, only on a page that fits the template, and only on a form that prints the locus on its values — the value's own prefix then corroborates the template's locus (gate 2), and geometry still decides.
- A printed label whose ruled cell is paper is NOT_TESTED, measured per page; the same status HA-009 gives a row a laboratory never fills. Ink no engine boxed keeps the cell with a person.
- Ink is glyph-shaped components, not dark pixels; and paper is only claimed on a page whose own known text the same measure can see (the control). Every ink decision is re-judged on a later run and withdrawn if it can no longer be measured.
