...
| **W3(b)** too-wide / wrong owner | **not built** | 8 of the 12 are `N candidate values exceeds max_values=2`: not a wide box to split but a choice of WHICH candidates the locus owns. That is surgery on `anchors.resolve_in_row`, the most load-bearing code here, and the project requires an adversarial review pass for it. Specified, not started |
